        ...
        <P class="text-center">{{ $user->nrp }}</P>
          <ul class="list-group list-group-flush">
            ...
            <table class="table table hover">
                <th>
                <a href="{{ route('user.ediProgram', $user->name) }}" class="badge bg-primary float-right mx-1"><span data-feather="edit-2"></span>edit</a>
                <form action="{{ route('Delete_Mahasiswa', $user->name) }}" method="post">
                    @csrf
                    @method('delete')
